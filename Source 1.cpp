	...
	while (1){
		...
		if (key == 'M'){//マッチテンプレート old version(メモリリークあり！！！）
			...
			if (num_effected != 0){
				...
				fprintf(file, "画像ファイル名,経過時間,t(s),x[pixel],y[pixel],X[meter],Y[meter],,ボールの直径(メートル)→,(ボールの直径を入力),テンプレート画像でのピクセルサイズ→,(テンプレート画像の横サイズを入力,1ピクセルあたりメートル出力→,=$J$1/$L$1\n");
				//各行の記述
...
void D00(void)		//キー操作一覧のテキスト表示
{
	...
	fprintf(stderr, " t :無制限に画像を保存(Spece Keyの入力で終了)\n");
